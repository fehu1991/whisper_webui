...
[回到專案首頁](../README.md) · [即時轉錄與保密操作](LIVE_TRANSCRIPTION.md)

以下 CMD 與 HTML 都位於解壓縮後的最外層。請保留完整資料夾，不要單獨搬出 CMD。

目前版本：`1.2.1`。新增 WhisperLiveKit 本機即時收音，收音與文字稿編輯分成兩頁，切頁不會停止收音。詳細步驟請看 [即時轉錄與保密操作說明](LIVE_TRANSCRIPTION.md)。
...
6. 如需「現場收音 → 即時轉錄 → 邊聽邊改文字」，再雙擊 `DeployLive.cmd` 安裝即時功能。
7. 雙擊 `RunWhisper.cmd` 啟動介面。選「即時收音／文字稿編輯」進入新工作區；原音檔轉錄功能仍在。
...
| 瀏覽器 | 音檔轉錄支援現代桌面瀏覽器；即時收音使用 Edge 或 Chrome | 原音檔頁建議視窗寬度至少 1280 px；即時頁可縮放 |
...
在 GitHub 專案首頁按綠色 `Code` 按鈕，再選 `Download ZIP`。
...
完成基本部署後，用這個工具安裝 WhisperLiveKit 即時功能，並準備 small 模型。此步驟會連網下載套件與模型，請在會議前完成。之後仍使用 `RunWhisper.cmd` 啟動，不必每次重新部署。NVIDIA 使用者可先完成下方 GPU 環境設定，再安裝即時功能。

即時模式預設選 small，裝置與精度由引擎自動判斷。CPU 也可執行，但是否跟得上現場語速必須實測。即時模式暫不自動區分說話人，可在編輯頁手動標註。
...
以下是原音檔轉錄功能的測試，不是 WhisperLiveKit 即時模式的效能或準確率保證。數據來自指定硬體與設定，實際速度仍會受到音檔內容、電腦負載、暫存資料與背景程式影響，因此其他電腦不一定會得到完全相同的時間。
...
- 獨立雙擊 HTML 時不會自動儲存；重新整理或關閉分頁後，尚未下載的修改會消失。從即時工作台開啟並建立或載入收音場次後，修改才會自動存至本機工作資料夾。兩種模式都不使用瀏覽器的 localStorage。
- 重要內容請先下載備份，再繼續大量修改。
...
| 介面顯示太窄 | 原音檔頁請將瀏覽器最大化；即時頁會隨視窗縮放，編輯表格必要時可左右捲動 |
...
| `.whisper` | 介面設定、即時文字稿、人工修改、選擇保留的錄音與音檔頁暫存 | 含工作資料，請依組織保留規範處理；不要當成可隨意清除的設定資料夾 |

發布版的 `.gitignore` 已排除上述本機資料，但無法阻止手動上傳、已追蹤的檔案或雲端同步。發布前務必檢查檔案清單，不要選取整個部署後的資料夾上傳。機密會議請依 [保密操作說明](LIVE_TRANSCRIPTION.md) 先完成離線驗證。
